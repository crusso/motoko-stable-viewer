...
│   └── declarations/
│       └── viewer_backend/
│           ├── viewer_backend.did.js  # Candid IDL factory (generated by dfx generate)
...
- The `src/declarations/` files (including `viewer_backend.did.js`) can be regenerated with `dfx generate` after deploying the backend.
